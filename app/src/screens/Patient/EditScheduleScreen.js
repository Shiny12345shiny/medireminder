...
const EditScheduleScreen = ({ route, navigation }) => {
  ...
  const loadSchedule = async () => {
    ...
    if (result.success) {
      // API interceptor returns response.data (the server payload {success, data})
      // ScheduleContext wraps it again as {success, data: response.data}
      // So result.data may be {success, data: schedule} or the schedule directly
      const schedule = result.data?.data || result.data;
      setFormData({
        medicineName: schedule.medicineName,
        medicineType: schedule.medicineType,
        dosage: {
          amount: schedule.dosage.amount.toString(),
          unit: schedule.dosage.unit
        },
        frequency: schedule.frequency,
        timings: schedule.timings,
        duration: {
          startDate: new Date(schedule.duration.startDate),
          endDate: schedule.duration.endDate ? new Date(schedule.duration.endDate) : null,
          isIndefinite: schedule.duration.isIndefinite
        },
        stock: {
          totalQuantity: schedule.stock.totalQuantity.toString(),
          remainingQuantity: schedule.stock.remainingQuantity.toString(),
          refillThreshold: (schedule.stock.lowStockThreshold ?? 5).toString()
        },
        instructions: {
          beforeFood: schedule.instructions?.beforeFood || false,
          afterFood: schedule.instructions?.afterFood || false,
          withFood: schedule.instructions?.withFood || false,
          emptyStomach: schedule.instructions?.emptyStomach || false,
          specialInstructions: schedule.instructions?.specialInstructions || ''
        },
        identification: {
          color: schedule.identification?.color || '',
          shape: schedule.identification?.shape || ''
        }
      });
    }
    setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    const updateData = {
      ...
      stock: {
        ...
        remainingQuantity: parseFloat(formData.stock.remainingQuantity),
        lowStockThreshold: parseInt(formData.stock.refillThreshold)
      }
    };
          ...
          <View style={styles.row}>
            <View style={[styles.inputGroup, { flex: 1, marginRight: 10 }]}>
              ...
              <TextInput
                ...
                placeholderTextColor="#999"
                keyboardType="numeric"
                value={formData.stock.remainingQuantity}
                onChangeText={(text) => updateNestedField('stock', 'remainingQuantity', text)}
              />
            </View>
